        ...
        while 1:
            for _ in tqdm(range(g.num_batch), total=g.num_batch, ncols=70, leave=False, unit='b'):
                ...
                if gs % 1000 == 0:
                    ...
                    # AS: setup parent checkpoints save dir
                    ljdir = Path(logdir)
                    root_cp_dir = ljdir.parent.joinpath("checkpoints")
                    try: 
                        root_cp_dir.mkdir()
                    except FileExistsError as e: 
                        pass
                    
                    # AS: move checkpoint and model files before new cp save
                    prev_stepnum_token = str( (gs - 1000) // 1000).zfill(3) + "k"
                    prev_cp_dir = root_cp_dir.joinpath(ljdir.name + "_" + prev_stepnum_token)
                    prev_cp_dir.mkdir() 
                    
                    prev_cp_model_files = glob.glob(str(ljdir.joinpath("model_gs_" + prev_stepnum_token)) + "*")
                    for cp_model_file  in prev_cp_model_files:
                        cp_model_file = Path(cp_model_file)
                        move_name = prev_cp_dir.joinpath(cp_model_file.name)
                        cp_model_file.rename(move_name)
                    
                    # AS: copy image an checkpoint file
                    prev_cp_image = ljdir.joinpath("alignment_" + prev_stepnum_token + ".png")
                    if prev_cp_image.is_file():
                        copyfile(prev_cp_image, prev_cp_dir.joinpath(prev_cp_image.name))
                    copyfile(ljdir.joinpath("checkpoint"), prev_cp_dir.joinpath("checkpoint"))
